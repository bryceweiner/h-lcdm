...
# Prepare the BAO-specific Dockerfile
cp -f "$DOCKER_DIR/Dockerfile.bao" "$DOCKER_DIR/Dockerfile"

IMAGE_TAG="hlcdm-bao"
docker build -t "$IMAGE_TAG" "$DOCKER_DIR"

# Default to running the BAO validation suite if no args provided
if [ "$#" -eq 0 ]; then
  set -- --bao validate --quiet
fi
